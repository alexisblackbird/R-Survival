...
new.daily <- function(name, type){ # Creates a new daily. Needs a type and name (short string)
  ...
  if(type == "walk" || type == "clean" || type == "mana" || type == "none"){
    stats[[paste("daily",stats$ndailies + 1,".done", sep="")]] <<- FALSE # Makes a new variable for numbered daily to log progress, puts it in stats so it will save - need to use the stats[[variable name]] <<- to get it to work for changing variable-name variables in stats rather than the global env, assign() is no good for this
    stats[[paste("daily",stats$ndailies + 1,".type", sep="")]] <<- type # Makes a new variable to store daily type
    stats[[paste("daily",stats$ndailies + 1,".name", sep="")]] <<- name # Makes a new variable to store daily name
    stats$ndailies <<- stats$ndailies + 1 # Updates the total number of dailies - for properly labelling new dailies, and later checking if they're all done
